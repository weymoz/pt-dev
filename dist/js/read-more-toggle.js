...
function toggleReadMore(evt) {
  ...
  evt.preventDefault(); //get div to open/close

  var readMoreDiv = this.previousElementSibling; //get the div to scroll to when closed

  var nextDiv = this.nextElementSibling; //if div is open ...

  if (readMoreDiv.style.maxHeight) {
    this.innerText = "Далее";
    readMoreDiv.style.maxHeight = null; //wait untill css animation allmost stopped and then scroll
